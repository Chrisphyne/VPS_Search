# Cell 5: Data Processor and Vector Store (BETTER PARSING)
"""
Process occurrence data for LLM consumption and create vector store for semantic search
IMPROVED: Better database result parsing that handles different result formats
"""
...
def parse_db_result_to_dict(db_result) -> List[Dict]:
    """Parse database result into list of dictionaries - IMPROVED VERSION"""
    try:
        # Handle different types of database results
        rows = []
        
        # If it's already a list of tuples or similar
        ...
        # If it's a string representation, try to parse it
            ...
            import ast
            try:
                rows = ast.literal_eval(db_result)
            except:
                print(f"Could not parse string result: {db_result[:100]}...")
                return []
        else:
            print(f"Unexpected result type: {type(db_result)}")
            print(f"Result content: {str(db_result)[:200]}...")
            return []
        ...
        print(f"Result: {str(db_result)[:300]}...")
        return []
...
print("✅ Data Processor and Vector Store (BETTER PARSING) initialized!")
